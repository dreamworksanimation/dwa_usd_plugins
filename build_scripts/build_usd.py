...
def InstallUSD(context, force, buildArgs):
    with CurrentWorkingDirectory(context.usdSrcDir):
        ...
        if context.buildNuke:
            if context.nukeLocation:
                extraArgs.append('-DNUKE_LOCATION="{nukeLocation}"'
                                 .format(nukeLocation=context.nukeLocation))
            extraArgs.append('-DPXR_BUILD_NUKE_PLUGIN=ON')
        else:
            extraArgs.append('-DPXR_BUILD_NUKE_PLUGIN=OFF')
...
group = parser.add_argument_group(title="Nuke Plugin Options")
subgroup = group.add_mutually_exclusive_group()
subgroup.add_argument("--nuke", dest="build_nuke", action="store_true", 
                      default=False,
                      help="Build Nuke plugin for USD")
subgroup.add_argument("--no-nuke", dest="build_nuke", action="store_false",
                      help="Do not build Nuke plugin for USD (default)")
group.add_argument("--nuke-location", type=str,
                   help="Directory where Nuke is installed.")
...
class InstallContext:
    def __init__(self, args):
        ...
        # - Nuke Plugin
        self.buildNuke = args.build_nuke
        self.nukeLocation = (os.path.abspath(args.nuke_location)
                                if args.nuke_location else None)
...
if not context.buildPython:
    ...
    if context.buildKatana:
        ...
        sys.exit(1)
    #if context.buildNuke:
    #    PrintError(pythonPluginErrorMsg % "Nuke")
    #    sys.exit(1)
...
summaryMsg = summaryMsg.format(
    ...
    buildNuke=("On" if context.buildNuke else "Off"))
