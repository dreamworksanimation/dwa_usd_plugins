...
/// @file zprender/VolumeShaderOp.h
///
/// @author Jonathan Egstad


#ifndef zprender_VolumeShaderOp_h
#define zprender_VolumeShaderOp_h
...
*/
class ZPR_EXPORT VolumeShaderOp : public VolumeShader,
                                  public DD::Image::Material
{
  ...
  public:
    const char* Class()     const { return "VolumeShaderOp"; }
    const char* node_help() const { return ""; }

    //! A parent node is not required.
    VolumeShaderOp(::Node* node=NULL);

    //! Must have a virtual destructor!
    virtual ~VolumeShaderOp();


    //!
    /*virtual*/ VolumeShaderOp* isVolumeShaderOp() { return this; }


    //!
    static const char* zpClass();

    /*! !!HACK ALERT!! This adds an invisible 'zpVolumeShaderOp' knob
        that's used to identify a VolumeShaderOp-derived Op to other plugins.
        ...
        pointer to VolumeShaderOp*.

        Atm if this knob doesn't exist then the _evaludate*() methods will
        not be called since the node will not be recognized as a VolumeShaderOp!
    */
    void addVolumeShaderOpIdKnob(DD::Image::Knob_Callback f);
...
// end of zprender/VolumeShaderOp.h
